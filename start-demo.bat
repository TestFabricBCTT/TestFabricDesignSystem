...
start "DC BFF" cmd /c "cd /d c:\Rodrigo\TestFabricDesignSystem\TestAgentFactoryDigitalChannels\bff && npm start"
timeout /t 2 >nul

REM 6. DigitalChannels Frontend
echo [6/8] Starting DigitalChannels Frontend (port 5173)...
start "DC Frontend" cmd /c "cd /d c:\Rodrigo\TestFabricDesignSystem\TestAgentFactoryDigitalChannels\frontend && npx vite --port 5173 --host"
timeout /t 2 >nul

REM 7. MCP API Server
echo [7/8] Starting MCP API Server (port 3001)...
start "MCP Server" cmd /c "cd /d c:\Rodrigo\TestFabricDesignSystem\Test-BCTT-Agent-Factory-MCPServer\mcp-server && npm run start:api"
timeout /t 2 >nul
