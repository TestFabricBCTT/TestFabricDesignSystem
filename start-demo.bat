...
echo   Fase 1 + Fase 2 Full Stack + Storybook
echo ===================================================================
echo.

REM 1. Docker (Nginx + SonarQube + Keycloak)
echo [1/9] Starting Docker containers (Nginx + SonarQube + Keycloak)...
...
echo [8/9] Starting Workshop Agent Factory (port 3000)...
start "Workshop" cmd /c "cd /d %~dp0Test-BCTT-Agent-Factory-MCPServer\workshop-agent-factory && npm run dev"
timeout /t 2 >nul

REM 9. Storybook (Design System)
echo [9/9] Starting Storybook — Design System (port 6006)...
start "Storybook" cmd /c "cd /d %~dp0Test-BCTT-Agent-Factory-MCPServer\bctt-design-system && npm run storybook"
timeout /t 3 >nul
...
echo.
echo   Storybook:        http://storybook.DCsCreatedbyAI.pt    (port 6006)
